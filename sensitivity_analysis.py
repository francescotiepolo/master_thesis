...
def _evaluate_base(params_row, seed): # For base model
    ...
    """
    nu, G, q, mu, beta_trade_off = params_row
...
def _evaluate_ps(params_row, seed): # For product space model (same as for base + PS params)
    ...
    """
    nu, G, q, mu, beta_trade_off, s, c, c_prime, gamma, kappa, sigma = params_row
    nan_out = np.full(len(OUTPUT_NAMES), np.nan)
    try:
...
def run_sobol(model_name, problem, eval_fn, n=N_SOBOL):
    n_runs = n * (2 * problem["num_vars"] + 2) # Sobol sampling formula
...
def plot_single_model(result, model_name):
    ...
    fig.suptitle(f"Sobol Sensitivity  {model_name}", fontsize=13, fontweight="bold", y=1.01)
...
def plot_comparison(result_base, result_ps):
    ...
    fig.suptitle("Shared Parameters: BaseModel vs ProductSpaceModel", fontsize=13, fontweight="bold")
...
def plot_ps_only(result_ps):
    ...
    fig.suptitle("ProductSpaceModel  PS-Only Parameter Sensitivity", fontsize=13, fontweight="bold")
    for out_name, ax in zip(OUTPUT_NAMES, axes):
        sp  = si_ps[out_name]
        c1, c2 = COLORS[out_name]
...
def plot_output_distributions(result_base, result_ps):
    ...
    fig.suptitle("Output Distributions Across Sobol Samples", fontsize=12, fontweight="bold")
    for k, out_name in enumerate(OUTPUT_NAMES):
...
if __name__ == "__main__":
    ...
    print(f"BaseModel runs     : {N_SOBOL * (2*PROBLEM_BASE['num_vars']+2)}")
    print(f"ProdSpaceModel runs: {N_SOBOL * (2*PROBLEM_PS['num_vars']+2)}")
