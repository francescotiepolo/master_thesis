...
- Choose option "CP_ONLY" to skip hysteresis sweep and only compute
  equilibrium C and P levels for each parameter set
"""
...
SECOND_ORDER = True
# CP_ONLY: skip hysteresis sweep, only compute equilibrium C and P
CP_ONLY = os.environ.get("CP_ONLY", "0") == "1"

# Configuration
N_SOBOL = 4096 # Saltelli draws N*(2D+2) samples per model
N_JOBS = int(os.environ.get("N_JOBS_OVERRIDE", -1))
BASE_SEED = 133 # Each sample gets BASE_SEED + sample_index
PROGRESS_CHUNK_MULT = int(os.environ.get("SA_PROGRESS_CHUNK_MULT", "8"))
STRUCTURE_SEED = int(os.environ.get("SA_STRUCTURE_SEED", str(BASE_SEED)))
...
T_INIT = 5_000 if CP_ONLY else 2_000  # Integration time for initial equilibrium
T_STEP = 2_000  # Integration time per d_C step

# Directory for results
ON_CLUSTER = os.environ.get("ON_CLUSTER", "0") == "1" # Flag if running on cluster
if CP_ONLY:
    _base_dir = "sa_cp_results"
elif FIX_Q_1:
    _base_dir = "sa_results_q_fix"
else:
    _base_dir = "sa_results"
...
HYSTERESIS_OUTPUTS = ["d_collapse", "d_recovery", "hysteresis_width"]
CP_OUTPUTS = ["mean_C_eq", "mean_P_eq"]
OUTPUT_NAMES = CP_OUTPUTS if CP_ONLY else (HYSTERESIS_OUTPUTS + CP_OUTPUTS)
...
COLORS = {
    ...
    "mean_C_eq": ("#7851A9", "#C3A6E0"),
    "mean_P_eq": ("#F07F09", "#F7C27A"),
}
...
def _extract_template(model):
    """Extract template for fast SA model setup."""
    active = model.adj_matrix > 0
    alpha = model.alpha.copy()
    alpha_safe = np.where(active, alpha, 1.0)
    beta0_P = np.where(active, model.beta_P * alpha_safe, 0.0)
    beta0_C = np.where(active, model.beta_C * alpha_safe, 0.0)
    template = {
        "adj_matrix": model.adj_matrix.copy(),
        "forbidden_network": model.forbidden_network.copy(),
        "KC": model.KC.copy(),
        "KP": model.KP.copy(),
        "alpha": alpha,
        "beta0_P": beta0_P,
    ...
    if hasattr(model, "phi_space"):
        template["phi_space"] = model.phi_space.copy()
    return template


def _get_base_template():
    """
    Get template for BaseModel to speed up SA by avoiding
    repeated random network generation and matrix construction.
    """
    ...
    if _BASE_TEMPLATE is None:
        with contextlib.redirect_stdout(io.StringIO()):
            m = BaseModel(
                ...
                nu=0.5, G=1.0, q=0.0, mu=1e-4,
                beta_trade_off=0.0,
                feasible=False, seed=STRUCTURE_SEED,
            )
        _BASE_TEMPLATE = _extract_template(m)
    return _BASE_TEMPLATE


def _get_ps_template():
    """
    Get template for ProductSpaceModel to speed up SA by avoiding
    repeated random network generation and matrix construction.
    """
    global _PS_TEMPLATE
    if _PS_TEMPLATE is None:
        with contextlib.redirect_stdout(io.StringIO()):
            m = ProductSpaceModel(
                N_products=15, n_countries=35,
                nestedness=FIXED_NESTEDNESS, connectance=FIXED_CONNECTANCE,
                ...
                beta_trade_off=0.0,
                feasible=False, seed=STRUCTURE_SEED,
                s=0.1, c=0.2, c_prime=0.2,
                gamma=1.0, kappa=0.1, sigma=1.0,
                enable_entry=True, entry_threshold=1.0,
            )
        _PS_TEMPLATE = _extract_template(m)
    return _PS_TEMPLATE


def _apply_template(m, template):
    """
    Apply a patch_network model from a cached deterministic template.
    """
    m.adj_matrix = template["adj_matrix"].copy()
    m.forbidden_network = template["forbidden_network"].copy()
    m.KC = template["KC"].copy()
    m.KP = template["KP"].copy()
    m.alpha = template["alpha"].copy()

    bt = m.beta_trade_off
    active = m.adj_matrix > 0
    alpha_safe = np.where(active, m.alpha, 1.0)
    KP_bt = np.maximum(m.KP, 1)[np.newaxis, :] ** bt
    KC_bt = np.maximum(m.KC, 1)[:, np.newaxis] ** bt
    with np.errstate(divide="ignore", invalid="ignore"):
        m.beta_P = np.where(active, template["beta0_P"] / (KP_bt * alpha_safe), 0.0)
        m.beta_C = np.where(active, template["beta0_C"] / (KC_bt * alpha_safe), 0.0)

    # These are overwritten immediately by _apply_shared_overrides
    m.C_PP = np.zeros((m.SP, m.SP), dtype=float)
    m.C_CC = np.zeros((m.SC, m.SC), dtype=float)
    m.h_P = np.ones(m.SP, dtype=float)
    m.h_C = np.ones(m.SC, dtype=float)
    m.r_P = np.ones(m.SP, dtype=float)
    m.r_C = np.ones(m.SC, dtype=float)

    if "phi_space" in template:
        m.phi_space = template["phi_space"].copy()
        m._build_product_space_matrices()
...
def _run_hysteresis(model):
    ...
    nan_out = {"d_collapse": np.nan, "d_recovery": np.nan, "hysteresis_width": np.nan,
               "mean_C_eq": np.nan, "mean_P_eq": np.nan}
    ...
    model.solve(
        ...
        save_period=0,
        save_trajectory=not CP_ONLY,
        max_solver_steps=50_000 if CP_ONLY else 10_000,
    )
    if model.y is None:
        return nan_out

    # Capture equilibrium C and P levels
    P_eq = model.y[:model.SP, -1]
    C_eq = model.y[model.SP:model.N, -1]
    mean_C_eq = float(np.mean(np.maximum(C_eq, 0.0)))
    mean_P_eq = float(np.mean(np.maximum(P_eq, 0.0)))

    # CP_ONLY mode: skip hysteresis sweep entirely
    if CP_ONLY:
        return {"mean_C_eq": mean_C_eq, "mean_P_eq": mean_P_eq}
    ...
    if not C_fwd:
        return {**nan_out, "mean_C_eq": mean_C_eq, "mean_P_eq": mean_P_eq}
    ...
    if not C_bwd:
        return {"d_collapse": d_collapse, "d_recovery": np.nan, "hysteresis_width": np.nan,
                "mean_C_eq": mean_C_eq, "mean_P_eq": mean_P_eq}
    ...
    try:
        ...
        with contextlib.redirect_stdout(io.StringIO()): # Suppress printouts from model
            m = BaseModel(
                ...
                feasible=False, seed=int(seed), patch_network=True,
            )
        _apply_template(m, template)
        _apply_shared_overrides(m, float(h_mean), float(r_mean), float(C_diag_mean), float(C_offdiag_mean))
        r = _run_hysteresis(m)
        return np.array([r[k] for k in OUTPUT_NAMES])
...
def run_sobol(model_name, problem, eval_fn, n=N_SOBOL):
    ...
    if CP_ONLY:
        ...
        chunk_size = max(1, n_jobs_eff * PROGRESS_CHUNK_MULT)
        print(
            f"Workers: {n_jobs_eff}  chunk_size: {chunk_size}  progress updates: every chunk",
            flush=True,
        )
        results = []
        t0 = time.time()
        for start in range(0, n_runs, chunk_size):
            ...
            # Force a fresh worker pool for the next chunk to avoid gradual
            # memory growth across many ProductSpaceModel evaluations.
            ...
            print(
                f"  [{model_name}] progress: {done}/{n_runs} ({100 * done / n_runs:.1f}%)"
...
def plot_single_model(result, model_name):
    """
    Bar charts for hysteresis outputs only. Skipped in CP_ONLY mode.
    """
    if CP_ONLY:
        return
    ...
    fig, axes = plt.subplots(len(outputs), 1,
                             figsize=(max(10, len(names) * 0.95), len(outputs) * 3.8))
    title_suffix = " - q = 1.0" if FIX_Q_1 else (" [second-order]" if SECOND_ORDER else "")
    fig.suptitle(f"Sobol Sensitivity — Hysteresis — {model_name}{title_suffix}",
                 fontsize=13, fontweight="bold", y=1.01)
    for out_name, ax in zip(outputs, axes):
        s = Si[out_name]
...
def plot_s2_heatmap(result, model_name):
    """
    Heatmap of second-order Sobol indices. Skipped if not SECOND_ORDER or in CP_ONLY mode.
    """
    if not SECOND_ORDER or CP_ONLY:
        return
    problem = result["problem"]
    names = problem["names"]
    n = len(names)
...
def plot_comparison(result_base, result_ps):
    """
    Comparison of shared parameter sensitivity between BaseModel and ProductSpaceModel.
    """
    if CP_ONLY:
        return
...
def plot_ps_only(result_ps):
    """
    Bar charts for ProductSpaceModel PS-only parameters. Skipped in CP_ONLY mode.
    """
    if CP_ONLY:
        return
    n_shared = len(SHARED_PARAMS["names"])
...
def plot_cp_outputs(result, model_name):
    """
    Separate bar charts showing how parameters influence equilibrium C and P levels.
    Saved as a distinct file from the hysteresis plots.
    """
    problem = result["problem"]
    names = problem["names"]
    Si = result["Si"]
    ...
    if len(outputs) == 1:
        axes = [axes]
    title_suffix = " - q = 1.0" if FIX_Q_1 else ""
    fig.suptitle(f"Sobol Sensitivity — Equilibrium C & P — {model_name}{title_suffix}",
                 fontsize=13, fontweight="bold", y=1.01)
    for out_name, ax in zip(outputs, axes):
        s = Si[out_name]
        c1, c2 = COLORS[out_name]
        _bar_pair(ax, names, s["S1"], s["S1_conf"], s["ST"], s["ST_conf"],
                  f"Output: {out_name}", c1, c2)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    path = os.path.join(RESULTS_DIR, f"{model_name}_sobol_CP.png")
    plt.savefig(path, dpi=150, bbox_inches="tight")
    print(f"Saved: {path}")
    plt.close()
...
def plot_output_distributions(result_base, result_ps):
    """
    Histograms comparing output distributions across all Sobol samples for BaseModel vs ProductSpaceModel.
    """
...
if __name__ == "__main__":
    ...
    plot_cp_outputs(result_base, "BaseModel")
    if SECOND_ORDER:
    ...
    plot_cp_outputs(result_ps, "ProductSpaceModel")
    if SECOND_ORDER:
