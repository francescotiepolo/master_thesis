...
def set_year_range(start_year, year_end=None):
    """
    Override the simulation year range. Rebuilds YEARS in place so existing
    references (default args, downstream imports) see the new values.
    """
    global SIM_START_YEAR, YEAR_END
    SIM_START_YEAR = int(start_year)
    if year_end is not None:
        YEAR_END = int(year_end)
    new_years = [
        y for y in range(SIM_START_YEAR, YEAR_END + 1) if y not in SKIP_YEARS
    ]
    YEARS.clear()
...
def _cache_key(mode_label, code, theta):
    ...
    # Include the year range so caches from different start/end years don't collide.
    hasher.update(f"{SIM_START_YEAR}-{YEAR_END}".encode("utf-8"))
    try:
...
def simulate_alpha_frozen(theta, data, country_idx, save_cb=None):
    ...
    pinned to observed at each year boundary. The annual ODE step is left
    unchanged, matching the simulator used during country-wise alpha-frozen
    calibration; at the boundary only C[target] is kept from the ODE and P is
    reconstructed.
...
# Inclusion statistic: calibration-style composite fit on the country market
# share, mirroring the alpha_frozen loss weights in calibration_country_wise
# (_get_mode_aware_loss_weights: nrmse_C=0.45, traj_corr_C=0.55). Lower is
# better; values are directly comparable to the calibration loss_total.
INCLUSION_W_NRMSE = 0.45
INCLUSION_W_TRAJ = 0.55


def compute_inclusion_stats(traj, country_idx, data):
    """
    Calibration-style composite fit statistic for one simulated trajectory.

    Mirrors compute_country_year_stats / aggregate_country_loss in
    calibration_country_wise.py restricted to the C-share terms:

      nrmse_C   : per-year |log share_sim - log share_obs| normalised by the
                  naive (uniform 1/SC) predictor, time-weighted by
                  1 + TIME_WEIGHT_SLOPE * (year - YEAR_START), averaged.
      spearman_C: Spearman correlation between the sim and obs share series.
      composite : INCLUSION_W_NRMSE * nrmse_C
                  + INCLUSION_W_TRAJ * (1 - spearman_C)

    The simulation start year is excluded (sim == obs by construction).
    Returns None if fewer than 3 usable years.
    """
    eps = 1e-10
    start_year = min(traj.keys())
    years = [
        y for y in sorted(traj.keys())
        if y in data["C_obs"] and y > start_year
    ]
    if len(years) < 3:
        return None

    nrmse_total, weight_total = 0.0, 0.0
    share_sim_series, share_obs_series = [], []
    for year in years:
        C_sim = traj[year]["C"]
        share_sim = float(C_sim[country_idx] / max(C_sim.sum(), 1e-12))
        ...
        nrmse_total += weight * nrmse_C
        weight_total += weight
        share_sim_series.append(share_sim)
        share_obs_series.append(share_obs)

    nrmse_C = nrmse_total / weight_total
    spearman_C = spearman(
        np.asarray(share_sim_series, dtype=float),
        np.asarray(share_obs_series, dtype=float),
    )
    composite = (
        INCLUSION_W_NRMSE * nrmse_C + INCLUSION_W_TRAJ * (1.0 - spearman_C)
    )
    # Endpoint gap: relative distance between simulated and observed market
    # share in the final simulated year. Combined with (1 - correlation) into
    # the selection score used to pick countries for the injection experiment.
    end_gap_rel = (
        abs(share_sim_series[-1] - share_obs_series[-1])
        / share_obs_series[-1]
        if share_obs_series[-1] > 0 else np.inf
    )
    selection_score = (1.0 - spearman_C) + end_gap_rel
    return {
        "composite": float(composite),
        "nrmse_C": float(nrmse_C),
        "spearman_C": float(spearman_C),
        "end_gap_rel": float(end_gap_rel),
...
def write_inclusion_stats_csv(per_country, country_rows, data, out_path):
    """Rank countries by the composite inclusion statistic and write a CSV."""
    rows_out = []
    for row in country_rows:
        ...
        if traj is None:
            continue
        st = compute_inclusion_stats(traj, row["position"], data)
        if st is None:
            continue
        rows_out.append({
            "country_code": code,
    ...
    rows_out.sort(key=lambda r: r["selection_score"])
    fields = [
        "rank", "country_code", "country_name", "selection_score",
        "spearman_C", "end_gap_rel", "composite", "nrmse_C",
        "n_years", "first_year", "last_year",
    ]
    with open(out_path, "w", newline="") as fh:
        writer = csv.DictWriter(fh, fieldnames=fields)
        writer.writeheader()
        for k, r in enumerate(rows_out, start=1):
            writer.writerow({"rank": k, **r})
...
def plot_trajectory_fit(per_country, country_rows, data, out_path, mode_label):
    ...
    for idx, row in enumerate(country_rows):
        ...
        st = compute_inclusion_stats(traj, ci, data)
        if st is not None:
            ax.text(
                0.02, 0.02,
                f"correlation = {st['spearman_C']:.2f}",
    ...
    fig.suptitle(
        ...
        "correlation = Spearman correlation between simulated and observed "
        "market-share trajectories (start year excluded)",
        fontsize=13,
        fontweight="bold",
    )
    plt.tight_layout()
...
def main():
    ...
    # Override module-level paths and year range so downstream code picks up
    # the CLI choices (cache keys include the year range as well).
    ...
    print(f"Years:      {SIM_START_YEAR}..{YEAR_END} (skipping {sorted(SKIP_YEARS)})")
    ...
    if "alpha_frozen" in modes:
        ...
        write_inclusion_stats_csv(
            frozen, rows, data,
            os.path.join(THIS_DIR, "inclusion_stats_alpha_frozen.csv"),
        )
